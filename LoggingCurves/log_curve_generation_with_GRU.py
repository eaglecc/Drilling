...
'''
    GRU预测伽马测井曲线
    分为训练集、测试集，然后可视化显示训练集和测试集上的预测效果
    参考：https://blog.csdn.net/m0_47256162/article/details/128595011
'''
...
plt.title("训练集预测结果")
plt.show()
# predict_train_data = scaler.inverse_transform((model(x_train_tensor).detach().numpy()).reshape(-1, 1))
# true_train_data = scaler.inverse_transform(y_train_tensor.detach().numpy().reshape(-1, 1))
