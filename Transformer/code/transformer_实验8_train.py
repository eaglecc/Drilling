...
def main(args):
    ...
    for i in range(0, len(data_y) - 100, 1):
        data_4_x.append(data_x[i:i + 100])
        data_4_y.append(data_y[i:i + 100])
    ...
    val_loader = Data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, drop_last=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, data in enumerate(train_loader):
            ...
            preds = model(conditions).reshape(32,100)
        ...
        with torch.no_grad():
            for step, data in enumerate(val_loader):
                ...
                outputs = list(preds.cpu().numpy().reshape([1, -1])[0])
        ...
        print("Epochs:[{}/{}],train loss:{:.4f},val loss:{:.4f}".format(epoch, args.epochs, train_loss_epoch,
                                                                        val_loss_epoch))
        # save best model
...
if __name__ == "__main__":
    ...
    parse.add_argument("--input_curves", default=["DENSITY", "NPHI", "VSHALE", "DPHI", "EPOR0"], type=list,
                       help="Type of input curves")
    parse.add_argument("--output_curves", default=["GR"], type=list,
    ...
    parse.add_argument("--epochs", type=int, default=50, help="Training epochs")
