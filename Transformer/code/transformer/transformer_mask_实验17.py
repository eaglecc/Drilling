...
data_x = data[['DENSITY', 'NPHI', 'DPHI', 'EPOR0', 'LITH']].values
...
model = Transformer(in_channels=5, out_channels=1, feature_num=64).to(device)
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel17.pth')
...
# 加载模型预测
model = Transformer(in_channels=5, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel17.pth'))
model.to(device)
model.eval()
