...
__project__:
原始Transformer对比实验
实验3: 钻前测井曲线预测实验：大庆油田井A数据集中  预测 BHC
__time__:  2024/3/9
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel_BHC.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel_BHC.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment33_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
# 11. 存储预测结果
# 反归一化
predicted_original_data = pred * (max_value_y - min_value_y) + min_value_y
test_target_original_data = test2 * (max_value_y - min_value_y) + min_value_y
file_name = '../../result/wlp_transformer/transformer_result_BHC.xlsx'
