...
__project__: 实验7:
WLP-Transformer
井径校正：大庆油田数据集 A井上 预测密度：DEN
使用井径数据划分数据集：扩径段、非扩径段
__time__:  2024/3/1
...
# 使用布尔索引过滤 井径数据
data = data[(data['CAL     .cm '] >= 22.7) & (data['CAL     .cm '] <= 23.2)]
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer7_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer7_DEN.pth'))
model.to(device)
model.eval()
# 在对模型进行评估时，应该配合使用wit torch.nograd() 与 model.eval()

test_all_data_x = test_all_data[['RMN-RMG', 'HAC     .us/m','GR      .   ',  'CAL     .cm ']].values
test_all_data_y = test_all_data['DEN     .g/cm3 '].values
min_value_y = test_all_data_y.min()  # 训练时y的最小值
max_value_y = test_all_data_y.max()  # 训练时y的最大值
#  Min-Max归一化
def min_max_normalize(data):
    min_vals = np.min(data, axis=0)  # 沿着列计算最小值
    max_vals = np.max(data, axis=0)  # 沿着列计算最大值
    normalized_data = (data - min_vals) / (max_vals - min_vals)
    return normalized_data
test_all_data_x = min_max_normalize(test_all_data_x)
test_all_data_y = min_max_normalize(test_all_data_y)

X, y = [], []
for i in range(len(test_all_data_x) - look_back + 1):
    X.append(test_all_data_x[i:i + look_back])
    y.append(test_all_data_y[i:i + look_back])
X = np.array(X)
y = np.array(y)

train_size1 = int(0.8 * len(X))
train_size2 = int(1 * len(X))
...
predicted2 = predicted[0, :, :].reshape(-1 , 1)   # 只取单行预测结果
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/wlp_experiment7_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
# data_depth = data_depth[train_size:train_size+future_window]
...
file_name = '../../result/wlp_transformer/daqingyoutian_result校正_DEN_8-10.xlsx'
