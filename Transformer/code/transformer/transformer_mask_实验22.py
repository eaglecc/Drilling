...
__project__: 实验22: 测井曲线补全实验：井B数据集中使用伽马射线值、中子孔隙度、光电因子、有效孔隙度  预测  密度
__time__:  2023/10/19
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel22_DENSITY.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel22_DENSITY.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment22_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment22_batch{}_epoch_{}.txt').format(batch_size, epochs)
