...
__project__:
原始Transformer对比实验
实验3: 钻前测井曲线预测实验：大庆油田井A数据集中  预测DEN
__time__:  2024/3/9
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel_DEN.pth'))
model.to(device)
model.eval()
...
file_name = '../../result/wlp_transformer/transformer_result_DEN.xlsx'
