...
__project__:
SD-Transformer对比实验
实验30: 钻前测井曲线预测实验：大庆油田井A数据集中  预测DEN
__time__:  2023/11/21
...
class Transformer(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, feature_num=128, res_num=4, encoder_num=4, use_pe=False,
        ...
        self.simple_decoder = Decoder(out_channels=out_channels, feature_num=feature_num, res_num=res_num)
    ...
    def forward(self, x):
        ...
        x = self.encode_in(x)
        # decoder
        # out = self.decode_out(tgt=target_in, memory=x)
        x = x.permute(1, 2, 0)
        out = self.simple_decoder(x)
        return out
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel35_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel35_DEN.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment35_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
print("MAPE:", mape)
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment35_batch{}_epoch_{}.txt').format(batch_size, epochs)
...
print("预测结果已写入experiment35_epoch_{}.txt文件")
...
file_name = '../../result/SD-transformer_daqingyoutian_result_DEN.xlsx'
