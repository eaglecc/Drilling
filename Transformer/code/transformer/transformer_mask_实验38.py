...
__project__:
eRPE-Transformer对比实验
实验38: 钻前测井曲线预测实验：大庆油田井A数据集中  预测DEN
__time__:  2024/2/28
...
# data_x = data[['RMN-RMG', 'SP      .mv  ', 'GR      .   ', 'CAL     .cm ', 'HAC     .us/m', 'BHC     .']].values
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        target_in = x
        x = x.permute(0, 2, 1)
        x = self.feature_embedding(x)  # 输入嵌入模块
        x = x.transpose(-2, -1)
        src = self.transformer_encdoer(x)
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel38_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel38_DEN.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment38_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
print("MAPE:", mape)
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment38_batch{}_epoch_{}.txt').format(batch_size, epochs)
...
print("预测结果已写入experiment38_epoch_{}.txt文件")
...
# file_name = '../../result/eRPE-transformer_daqingyoutian_result_BHC.xlsx'
# # 如果文件不存在，创建一个新 Excel 文件并存储数据
# df = pd.DataFrame({'well1_BHC_predicted': predicted_original_data.flatten()})  # 创建一个新 DataFrame
# df['well1_BHC_true'] = test_target_original_data
# df.to_excel(file_name, index=False)  # index=False 防止写入索引列
# mse = np.mean((test_target_original_data - predicted_original_data) ** 2)
# rmse = np.sqrt(np.mean((test_target_original_data - predicted_original_data) ** 2))
# mae = np.mean(np.abs(test_target_original_data - predicted_original_data))
# mape = np.mean(np.abs((test_target_original_data - predicted_original_data) / test_target_original_data))
# print("MSE", mse)
# print("RMSE", rmse)
# print("MAE", mae)
# print("MAPE:", mape)
