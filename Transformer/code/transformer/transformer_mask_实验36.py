...
实验36: 钻前测井曲线预测实验：大庆油田井A数据集中  预测DEN
__time__:  2023/11/22
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel36_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel36_DEN.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment36_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
print("MAPE:", mape)
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment36_batch{}_epoch_{}.txt').format(batch_size, epochs)
...
print("预测结果已写入experiment36_epoch_{}.txt文件")
...
file_name = '../../result/eRPE-transformer_daqingyoutian_result_DEN.xlsx'
