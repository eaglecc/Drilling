...
__project__: 实验2:
WLP-Transformer
钻前测井曲线预测：大庆油田数据集 A井 上 进行未来测井曲线预测 预测密度：BHC
__time__:  2024/3/1
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer1_BHC.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer1_BHC.pth'))
model.to(device)
model.eval()
...
plt.title('WLP-Transformer测井曲线预测')
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/wlp_experiment2_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
file_name = '../../result/wlp_transformer/daqingyoutian_result_BHC.xlsx'
# 如果文件不存在，创建一个新 Excel 文件并存储数据
df = pd.DataFrame({'depth': data_depth[:,0]})  # 创建一个新 DataFrame
df['well1_BHC_predicted'] = predicted_original_data
df['well1_BHC_true'] = test_target_original_data.flatten()
