...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer8_BHC.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer8_BHC.pth'))
model.to(device)
model.eval()
# 在对模型进行评估时，应该配合使用wit torch.nograd() 与 model.eval()
test_all_data_x = test_all_data[['RMN-RMG', 'HAC     .us/m' ,'SP      .mv  ',  'CAL     .cm ']].values
test_all_data_y = test_all_data['BHC     .'].values
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/wlp_experiment8_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
file_name = '../../result/wlp_transformer/daqingyoutian_result校正_BHC_8-10.xlsx'
