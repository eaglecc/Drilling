...
# 1. 导入数据
data = pd.read_csv('../../data/daqingyoutian/vertical_all_A5.csv')
...
data_x = data[['RMN-RMG', 'HAC     .none','GR      .none ',  'CAL     .none ']].values

data_y = data['DEN     .none'].values
input_features = 4
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer1_E_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer1_E_DEN.pth'))
model.to(device)
model.eval()
...
plt.plot(test2, label='True')
plt.plot(predicted2, label='Predicted')
plt.title('Transformer测井曲线预测')
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/wlp_experiment1_batch{}_E_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()

# 10. 计算深度
data_depth = data[['DEPT    .M    ','DEN     .none']].values
...
# 11. 存储预测结果
# 反归一化
predicted_original_data = predicted2 * (max_value_y - min_value_y) + min_value_y
test_target_original_data = test2 * (max_value_y - min_value_y) + min_value_y
file_name = '../../result/wlp_transformer/daqingyoutian_result_E_DEN.xlsx'
