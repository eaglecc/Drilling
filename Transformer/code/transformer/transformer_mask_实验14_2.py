...
__project__: 实验14：用单个井的数据集做集成学习实验   此部分仅预测不训练
__time__:  2023/09/21
...
if max_percentage == percentage_0:
    ...
    model.load_state_dict(torch.load('./pth/SingleTransformer_trainModel_LITH2.pth'))
else:
    model.load_state_dict(torch.load('./pth/SingleTransformer_trainModel_LITH3.pth'))
