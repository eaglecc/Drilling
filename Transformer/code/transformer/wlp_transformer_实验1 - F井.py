...
# 1. 导入数据
data = pd.read_csv('../../data/daqingyoutian/vertical_all_A6.csv')
...
data_x = data[['RMN-RMG', 'HAC     .non','GR      .none',  'CAL     .none ']].values
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer1_F_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer1_F_DEN.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/wlp_experiment1_batch{}_F_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
file_name = '../../result/wlp_transformer/daqingyoutian_result_F_DEN.xlsx'
