...
__project__:
原始Transformer对比实验
实验30: 钻前测井曲线预测实验：大庆油田井A数据集中  预测SP
__time__:  2023/11/2
...
data_x = data[['RMN-RMG', 'CAL     .cm ', 'GR      .   ', 'DEN     .g/cm3 ', 'HAC     .us/m', 'BHC     .']].values
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel31_SP.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=128).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel31_SP.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment31_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
print("MAPE:", mape)
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment31_batch{}_epoch_{}.txt').format(batch_size, epochs)
...
print("预测结果已写入experiment31_epoch_{}.txt文件")
...
file_name = '../../result/transformer_daqingyoutian_result_SP.xlsx'
