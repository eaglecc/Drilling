...
__project__: 实验27: 钻前测井曲线预测：大庆油田数据集 A井 上 进行未来测井曲线预测 预测SP
__time__:  2023/10/19
...
data_x = data[['RMN-RMG', 'GR      .   ', 'CAL     .cm ', 'DEN     .g/cm3 ', 'HAC     .us/m', 'BHC     .']].values
data_y = data['SP      .mv  '].values
input_features = 6
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_Transformer_trainModel27_SP.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_Transformer_trainModel27_SP.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../../result/transformer/experiment27_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
...
# 创建一个txt文件并将结果写入其中
resultpath = ('../../result/transformer/experiment27_batch{}_epoch_{}.txt').format(batch_size, epochs)
...
print("预测结果已写入experiment27_epoch_{}.txt文件")
...
file_name = '../../result/daqingyoutian_result_SP.xlsx'
# 如果文件不存在，创建一个新 Excel 文件并存储数据
df = pd.DataFrame({'well1_SP_predicted': predicted_original_data.flatten()})  # 创建一个新 DataFrame
df['well1_SP_true'] = test_target_original_data
