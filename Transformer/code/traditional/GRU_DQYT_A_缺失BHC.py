...
__project__: GRU进行大庆油田 A井上  缺失测井曲线预测实验  DEN
__time__:  2024/4/2
...
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        out, _ = self.gru(x)  # （3050，50，6）-->（3050，50，64）
...
file_name = '../../result/wlp_transformer/缺失预测A_gru_BHC_result.xlsx'
df = pd.DataFrame()  # 创建一个新 DataFrame
df['gru_BHC_predicted'] = predicted_original_data
df['gru_BHC_true'] = test_target_original_data.flatten()
