...
data_x = data[['NPHI', 'DENSITY', 'PEF', 'GR', 'DEPTH']]
data_y = data['EPOR0']
# data_y = data['DENSITY']
# data_y = data['EPOR0']
...
if os.path.exists(file_name):
    ...
    df['lstm_EPOR0_predicted'] = predicted
    df['lstm_EPOR0_true'] = test_target
    # 写入 DataFrame 到 Excel 文件
    df.to_excel(file_name, index=False)  # index=False 防止写入索引列
else:
    # 如果文件不存在，创建一个新 Excel 文件并存储数据
    df = pd.DataFrame({'lstm_EPOR0_predicted': predicted.flatten()})  # 创建一个新 DataFrame
    df['lstm_EPOR0_true'] = test_target
    df.to_excel(file_name, index=False)  # index=False 防止写入索引列
