...
data_x = data[['RMN-RMG', 'SP      .mv  ', 'CAL     .cm ', 'DEN     .g/cm3 ', 'HAC     .us/m', 'BHC     .']].values
...
file_name = '../../result/daqingyoutian_GRU_GR_result.xlsx'
# 如果文件不存在，创建一个新 Excel 文件并存储数据
df = pd.DataFrame({'gru_GR_predicted': predicted_original_data.flatten()})  # 创建一个新 DataFrame
df['gru_GR_true'] = test_target_original_data
