...
__project__: 实验2:
WLP-Transformer
缺失 测井曲线预测：美国油田数据集 A井 上 进行缺失测井曲线预测 预测密度：PE
__time__:  2024/3/1
...
data_x = data.iloc[:, 4:8].values  # 特征列
data_y = data.iloc[:, 8].values  # 标签列
input_features = 4
...
if train_model:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_epoch_loss < best_test_loss:
            ...
            torch.save(best_model.state_dict(), '../pth/best_WLP_Transformer2_DEN.pth')
...
# 加载模型预测
model = Transformer(in_channels=input_features, out_channels=1, feature_num=64).to(device)
model.load_state_dict(torch.load('../pth/best_WLP_Transformer2_DEN.pth'))
model.to(device)
model.eval()
...
plt.legend()
# 使用savefig保存图表为文件
plt.savefig(('../result/wlp_experiment2_batch{}_epoch_{}.png').format(batch_size, epochs))  # 保存为PNG格式的文件
plt.show()
